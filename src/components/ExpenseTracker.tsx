...
const mockExpenses = [
  { id: 1, description: "Groceries", amount: 85.5, category: "Food", date: "2024-01-10" },
  { id: 2, description: "Gas Station", amount: 45.0, category: "Transportation", date: "2024-01-09" },
  { id: 3, description: "Netflix Subscription", amount: 15.99, category: "Entertainment", date: "2024-01-08" },
  { id: 4, description: "Coffee Shop", amount: 12.5, category: "Food", date: "2024-01-08" },
...
const ExpenseTracker = () => {
  ...
  const [budget, setBudget] = useState<number>(() => {
    return Number(localStorage.getItem("budget")) || 2000;
  });
  ...
  // ---------- Reset Expenses & Budget ----------
  const resetAll = () => {
    if (window.confirm("Are you sure you want to delete all expenses and reset your budget?")) {
      setExpenses([]);
      setBudget(0);
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center gap-4 max-w-md">
          ...
          <Button
            onClick={resetAll}
            className="px-4 py-2 rounded-md text-white font-semibold shadow-button gradient-danger"
          >
            Reset All
          </Button>
        </div>
        ...
        {/* Expense Form Modal */}
        {showForm && <ExpenseForm onSubmit={addExpense} onClose={() => setShowForm(false)} />}
      </div>
    </div>
  );
};
